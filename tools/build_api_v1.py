...
def main():
    ...
    for rid in ids:
        data = load_json(f"data/{rid}.jsonld")
        ch = data.get("contentHash", "sha256:REPLACED_AT_RUNTIME").strip()
        write_json(f"api/v1/bundle/{rid}.json", {
            ...
            "latestAudit": load_json("audit/logs/index.json")["latest"]
        })

    # 4) manifest (sealed hashes of canonical bytes, using placeholder rule)
    manifest_recs = {}
    for rid in ids:
        obj = load_json(f"data/{rid}.jsonld")
        # enforce placeholder presence to match v1 rule
        if "contentHash" not in obj:
            obj["contentHash"] = "sha256:REPLACED_AT_RUNTIME"
